# Kubernetes Service Account for the AWS Load Balancer Controller
...
resource "helm_release" "aws_load_balancer_controller" {
  ...
  set {
    ...
    value = "false" # Service account is already created above
  }
  set {
    name  = "vpcId"
    value = var.vpc_id
  ...
  depends_on = [aws_iam_role_policy_attachment.lbc_custom_policy_attachment, kubernetes_service_account.aws_load_balancer_controller]
}
